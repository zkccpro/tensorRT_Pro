...
MeshGridPluginDynamic::MeshGridPluginDynamic(const std::string name,
  ...
  float *start_data =
      (float *)deserToHost<char>(d, start_length * sizeof(float));
  mStarts = std::vector<float>(&start_data[0], &start_data[0] + start_length);
  float *stride_data =
      (float *)deserToHost<char>(d, stride_length * sizeof(float));
  mStrides =
  ...
  if (pos == 0) {
    return (in[pos].type == nvinfer1::DataType::kFLOAT &&
           ...
           (in[pos].type == nvinfer1::DataType::kHALF &&
            in[pos].format == nvinfer1::TensorFormat::kLINEAR) ||
           (in[pos].type == nvinfer1::DataType::kINT32 &&
            in[pos].format == nvinfer1::TensorFormat::kLINEAR);
  } else {
...
int MeshGridPluginDynamic::enqueue(const nvinfer1::PluginTensorDesc *inputDesc,
  ...
  if (mNumInputs > 0) {
    ...
    for (int i = 0; i < mNumInputs; ++i) {
      ...
      switch (data_type) {
        case nvinfer1::DataType::kFLOAT:
          amirstan::cuda::repeat_dims<float>(
              (float *)outputs[i], (float *)inputs[i], &(input_dims.d[0]),
              &repeat_dims.d[0], input_dims.nbDims, stream);
          break;

        case nvinfer1::DataType::kHALF:
          amirstan::cuda::repeat_dims<half>(
              (half *)outputs[i], (half *)inputs[i], &(input_dims.d[0]),
              &repeat_dims.d[0], input_dims.nbDims, stream);
          break;

        case nvinfer1::DataType::kINT32:
          amirstan::cuda::repeat_dims<int>(
              (int *)outputs[i], (int *)inputs[i], &(input_dims.d[0]),
              &repeat_dims.d[0], input_dims.nbDims, stream);
          break;
        default:
          return 1;
    ...
    for (int i = 0; i < num_output; ++i) {
      ...
      switch (data_type) {
        case nvinfer1::DataType::kFLOAT:
          amirstan::plugin::arange_mesh_grid<float>(
              (float *)outputs[i], &(output_dims.d[0]), output_dims.nbDims, i,
              start, stride, stream);
          break;

        case nvinfer1::DataType::kHALF:
          amirstan::plugin::arange_mesh_grid<half>(
              (half *)outputs[i], &(output_dims.d[0]), output_dims.nbDims, i,
              start, stride, stream);
          break;

        case nvinfer1::DataType::kINT32:
          amirstan::plugin::arange_mesh_grid<int>(
              (int *)outputs[i], &(output_dims.d[0]), output_dims.nbDims, i,
              start, stride, stream);
          break;
        default:
          return 1;
...
IPluginV2 *MeshGridPluginDynamicCreator::createPlugin(
  ...
  for (int i = 0; i < fc->nbFields; i++) {
    ...
    if (field_name.compare("slice_dims") == 0) {
      ...
      memcpy(&(sliceDims.d[0]), static_cast<const int *>(fc->fields[i].data),
  ...
  MeshGridPluginDynamic *plugin =
      new MeshGridPluginDynamic(name, numInputs, sliceDims, starts, strides);
  plugin->setPluginNamespace(getPluginNamespace());
  return plugin;
}
