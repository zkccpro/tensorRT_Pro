...
template <typename T, typename AccT, typename ModifyOp, typename ReduceOp,
          typename FinalizeOp>
bool reduce2DContigous(T *out, const T *in, AccT init, size_t inElements,
  ...
  reduceContigous2DKernel<T, AccT, ModifyOp, ReduceOp, FinalizeOp>
      <<<grid, block, smemSize, stream>>>(out, in, reductionSize, outElements,
                                          init, modifyOp, reduceOp, finalizeOp);
