...
void arange_mesh_grid(T *output, const int *output_dims, int nb_dims,
  ...
  arange_mesh_grid_kernel<T><<<GET_BLOCKS(N), CUDA_NUM_THREADS, 0, stream>>>(
      output, pre_stride, post_stride, start, stride, N);
}

template void arange_mesh_grid<float>(float *output, const int *output_dims,
                                      int nb_dims, int slice_dim, float start,
                                      float stride, cudaStream_t stream);

template void arange_mesh_grid<int>(int *output, const int *output_dims,
                                    int nb_dims, int slice_dim, float start,
                                    float stride, cudaStream_t stream);

template void arange_mesh_grid<half>(half *output, const int *output_dims,
                                     int nb_dims, int slice_dim, float start,
                                     float stride, cudaStream_t stream);
