...
pluginStatus_t nmsInference(
    ...
    const void* confData, void* keepCount, void* nmsedBoxes, void* nmsedScores,
    void* nmsedClasses, void* workspace, bool isNormalized, bool confSigmoid,
    bool clipBoxes) {
  ...
  // cudaMemcpyAsync(nmsedScores, scores, topK*sizeof(float),
  ...
  status = gatherNMSOutputs(
      stream, shareLocation, N, numPredsPerClass, numClasses, topK, keepTopK,
      DataType::kFLOAT, DataType::kFLOAT, indices, scores, bboxData, keepCount,
      nmsedBoxes, nmsedScores, nmsedClasses, clipBoxes);
