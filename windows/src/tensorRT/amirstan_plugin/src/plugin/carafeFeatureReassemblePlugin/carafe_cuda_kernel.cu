...
int CARAFEForwardLaucher(const T *features, const T *masks,
  ...
  BatchTranspose2DCUDAKernel<T>
      <<<batch_size * dh * dw, dim3(kTileDim, kBlockRows), 0, stream>>>(
          batch_size, channels, input_height * input_width, dh, dw, features,
          rfeatures);
  ...
  const int block_size =
      (num_kernels + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
  CARAFEForward<T><<<block_size, THREADS_PER_BLOCK, 0, stream>>>(
      num_kernels, rfeatures, rmasks, kernel_size, group_size, scale_factor,
      channels, input_height, input_width, output_height, output_width,
      mask_channels, routput);

  dh = divideUP(output_height * output_width, kTileDim);
  dw = divideUP(channels, kTileDim);
  BatchTranspose2DCUDAKernel<T>
      <<<batch_size * dh * dw, dim3(kTileDim, kBlockRows), 0, stream>>>(
          batch_size, output_height * output_width, channels, dh, dw, routput,
          output);

  return 0;
}

template int CARAFEForwardLaucher<float>(
    const float *features, const float *masks, const int kernel_size,
    const int group_size, const int scale_factor, const int batch_size,
