...
class RoiPoolPluginDynamic : public PluginDynamicBase {
 public:
  RoiPoolPluginDynamic(const std::string &name, int outSize,
                       const std::vector<float> &featmapStrides,
                       float roiScaleFactor = -1, int finestScale = 56);

  RoiPoolPluginDynamic(const std::string name, const void *data, size_t length);

  // It doesn't make sense to make RoiPoolPluginDynamic without arguments, so we
  // delete default constructor.
  RoiPoolPluginDynamic() = delete;
...
class RoiPoolPluginDynamicCreator : public PluginCreatorBase {
 public:
  RoiPoolPluginDynamicCreator();
