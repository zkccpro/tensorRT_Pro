...
template <typename scalar_t>
__device__ scalar_t roi_pool_single(
...
template <typename scalar_t>
__global__ void roi_pool_kernel(scalar_t *output, const scalar_t *bottom_rois,
                                FeatData feat_data,
  ...
  CUDA_KERNEL_LOOP(index, nThreads) {
    ...
    const scalar_t output_val = roi_pool_single<scalar_t>(
        bottom_data, roi_batch_ind, roi_start_w, roi_start_h, roi_end_w,
        roi_end_h, spatial_scale, pw, ph, c, channels, height, width,
...
template <typename T>
void roi_pool(T *output, const T *rois, int num_rois, const void *const *feats,
              int num_feats, int n, int c, int *h, int *w, float *strides,
              int out_size, float roi_scale_factor, int finest_scale,
              cudaStream_t stream) {
  FeatData feat_data;
  ...
  roi_pool_kernel<T><<<GET_BLOCKS(nThreads), CUDA_NUM_THREADS, 0, stream>>>(
      output, rois, feat_data, roi_scale_factor, finest_scale, pooled_height,
      pooled_width, nThreads);
}

template void roi_pool<float>(float *output, const float *rois, int num_rois,
