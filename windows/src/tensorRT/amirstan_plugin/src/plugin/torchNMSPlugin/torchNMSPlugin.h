...
class TorchNMSPluginDynamic : public PluginDynamicBase {
 public:
  TorchNMSPluginDynamic(const std::string &name, float iouThreshold);

  TorchNMSPluginDynamic(const std::string name, const void *data,
                        size_t length);

  // It doesn't make sense to make TorchNMSPluginDynamic without arguments, so
...
class TorchNMSPluginDynamicCreator : public PluginCreatorBase {
 public:
