...
class RoiExtractorPluginDynamic : public PluginDynamicBase {
 public:
  RoiExtractorPluginDynamic(const std::string &name, int outSize, int sampleNum,
                            const std::vector<float> &featmapStrides,
                            float roiScaleFactor = -1, int finestScale = 56,
                            bool aligned = false);

  RoiExtractorPluginDynamic(const std::string name, const void *data,
                            size_t length);

  // It doesn't make sense to make RoiExtractorPluginDynamic without arguments,
...
class RoiExtractorPluginDynamicCreator : public PluginCreatorBase {
 public:
