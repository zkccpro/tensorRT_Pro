...
template <typename T>
void roi_extractor(T *output, const T *rois, int num_rois,
                   const void *const *feats, int num_feats, int n, int c,
                   int *h, int *w, float *strides, int out_size, int sample_num,
  ...
  roi_extractor_kernel<T>
      <<<GET_BLOCKS(nThreads), CUDA_NUM_THREADS, 0, stream>>>(
          output, rois, feat_data, sample_num, roi_scale_factor, finest_scale,
          pooled_height, pooled_width, aligned, nThreads);
}

template void roi_extractor<float>(float *output, const float *rois,
                                   int num_rois, const void *const *feats,
                                   int num_feats, int n, int c, int *h, int *w,
                                   float *strides, int out_size, int sample_num,
