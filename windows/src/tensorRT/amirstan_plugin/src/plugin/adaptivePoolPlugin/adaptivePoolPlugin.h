...
class AdaptivePoolPluginDynamic : public PluginDynamicBase {
 public:
  AdaptivePoolPluginDynamic(const std::string &name,
                            const nvinfer1::Dims &outputSize, int poolingType);

  AdaptivePoolPluginDynamic(const std::string name, const void *data,
                            size_t length);

  // It doesn't make sense to make AdaptivePoolPluginDynamic without arguments,
...
class AdaptivePoolPluginDynamicCreator : public PluginCreatorBase {
 public:
