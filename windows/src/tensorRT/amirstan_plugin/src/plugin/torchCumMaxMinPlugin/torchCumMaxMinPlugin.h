...
class TorchCumMaxMinPluginDynamic : public PluginDynamicBase {
 public:
  TorchCumMaxMinPluginDynamic(const std::string &name, int dim, int cumType);

  TorchCumMaxMinPluginDynamic(const std::string name, const void *data,
                              size_t length);

  // It doesn't make sense to make TorchCumMaxMinPluginDynamic without
  // arguments, so we delete default constructor.
  TorchCumMaxMinPluginDynamic() = delete;
...
class TorchCumMaxMinPluginDynamicCreator : public PluginCreatorBase {
 public:
  TorchCumMaxMinPluginDynamicCreator();
